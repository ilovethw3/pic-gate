...
def render_template(request: Request, name: str, context: dict | None = None):
    """Render a template using the current Starlette/FastAPI call signature."""
...
async def cache_page(
    ...
    return render_template(request, "cache.html", {
        "username": get_username(request)
    })
...
async def images_page(
    ...
    return render_template(request, "images.html", {
        "username": get_username(request)
    })
